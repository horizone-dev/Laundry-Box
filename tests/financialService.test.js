...
function setup() {
  ...
  db.exec(`
    ...
    CREATE TABLE payments (id TEXT PRIMARY KEY, customerId TEXT, orderId TEXT, shopId TEXT, amount REAL, method TEXT, status TEXT, createdAt TEXT, isSynced INTEGER, updatedAt TEXT, paymentReference TEXT);
...

{
  ...
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM payments WHERE paymentReference LIKE 'SETDISC-%'").get().count, 1);
  assert.equal(db.prepare("SELECT id FROM payments WHERE method = 'Cash'").get().id, 'RV-000001');
  assert.equal(db.prepare("SELECT id FROM payments WHERE method = 'Discount'").get().id.startsWith('SETDISC-'), true);
...

{
  ...
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM payments WHERE paymentReference LIKE 'DISC-%'").get().count, 1);
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM payments WHERE paymentReference LIKE 'SETDISC-%'").get().count, 0);
  db.close();
}
