...
export default function Invoice() {
  ...
  const executeNativePrint = async (forceSilent = null, printerType = 'billing', copies = 1) => {
    console.log("[Invoice.jsx] executeNativePrint entered: printerType =", printerType, "copies =", copies, "hasOrder =", !!order, "hasElectronAPI =", !!electronAPI?.printInvoice);
  ...
  useEffect(() => {
    console.log("[Invoice.jsx] Auto-print effect checked: id =", id, "settings.isLoaded =", settings?.isLoaded, "hasAutoPrinted =", hasAutoPrintedRef.current, "orderLoaded =", !!order);
    ...
    console.log("[Invoice.jsx] printParam =", printParam, "isPos =", isPos, "shouldPrint =", shouldPrint, "shouldDownload =", shouldDownload);
    if (order) {
      if (shouldPrint) {
        hasAutoPrintedRef.current = true;
        const copies = isPos ? Math.max(1, parseInt(settings.autoPrintCopies || 1, 10)) : 1;
        console.log("[Invoice.jsx] Triggering native print immediately with copies =", copies);
        executeNativePrint(null, 'billing', copies);
      } else if (shouldDownload) {
        hasAutoPrintedRef.current = true;
        console.log("[Invoice.jsx] Triggering PDF generation immediately");
        generatePDF();
      }
    }
